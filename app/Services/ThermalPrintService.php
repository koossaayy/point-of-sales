...
class ThermalPrintService
{
    public function generateReceiptText(Transaction $transaction, string $paperSize = '80mm'): string
    {
        ...
        if ($storePhone) {
            $lines[] = $this->center(__('Telp: ').$storePhone, $maxWidth);
        }
        $lines[] = $this->line($maxWidth);
        $lines[] = $this->left(__('No: ').($transaction->invoice ?? ''), $maxWidth);
        $lines[] = $this->left(__('Tgl: ').($transaction->created_at?->format('d/m/Y H:i') ?? ''), $maxWidth);
        $lines[] = $this->left(__('Kasir: ').($transaction->cashier?->name ?? '-'), $maxWidth);
        $lines[] = $this->left(__('Pelanggan: ').($transaction->customer?->name ?? __('Umum')), $maxWidth);
        $lines[] = $this->line($maxWidth);
