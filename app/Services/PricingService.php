...
class PricingService
{
    ...
    public function ruleLabel(PricingRule $rule): string
    {
        return match ($rule->kind) {
            PricingRule::KIND_QTY_BREAK => __('Grosir ').$this->standardDiscountLabel($rule),
            PricingRule::KIND_BUNDLE_PRICE => __('Bundle Rp ').number_format((float) $rule->discount_value, 0, ',', '.'),
    ...
    private function standardDiscountLabel(PricingRule $rule): string
    {
        return match ($rule->discount_type) {
            PricingRule::TYPE_PERCENTAGE => rtrim(rtrim(number_format((float) $rule->discount_value, 2, '.', ''), '0'), '.').__('% OFF'),
            PricingRule::TYPE_FIXED_AMOUNT => __('Hemat Rp ').number_format((float) $rule->discount_value, 0, ',', '.'),
            PricingRule::TYPE_FIXED_PRICE => __('Harga Rp ').number_format((float) $rule->discount_value, 0, ',', '.'),
            default => $rule->name,
        };
    }
