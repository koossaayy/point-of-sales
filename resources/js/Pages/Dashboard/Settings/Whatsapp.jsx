...
export default function Whatsapp({ settings, waStatus }) {
    ...
    const handleConnect = async () => {
        try {
            ...
            toast.error(t('Gagal menghubungkan'));
        }
    };
    ...
    const handleDisconnect = async () => {
        try {
            ...
            toast.success(t('Koneksi diputuskan'));
        } catch (e) {
            toast.error(t('Gagal memutuskan koneksi'));
        }
    };
    ...
    const handleSave = (e) => {
        ...
        post(route("settings.whatsapp.update"), {
            preserveScroll: true,
            onSuccess: () => toast.success(t('Pengaturan WhatsApp disimpan')),
            onError: () => toast.error(t('Gagal menyimpan')),
        });
    };

    const handleTest = async () => {
        if (!testNumber) return toast.error(t('Masukkan nomor tujuan'));
        try {
            await axios.post(route("settings.whatsapp.test"), { target: testNumber });
            toast.success(t('Pesan test terkirim!'));
        } catch (e) {
            toast.error(t('Gagal mengirim'));
        }
    };

    return (
        <>
            <Head title={t('Pengaturan WhatsApp')} />
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        {t('Hubungkan WhatsApp untuk kirim pesan otomatis via campaign CRM')}
                    </p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 max-w-lg">
                    ...
                    {status.qr && !status.connected && (
                        <div className="mb-4 text-center">
                            <img src={status.qr} alt={t('QR Code')} className="mx-auto w-48 h-48" />
                            <p className="text-xs text-slate-400 mt-2">
                                {t('Scan dengan WhatsApp > Perangkat Tertaut > Perangkat Baru')}
                            </p>
                        </div>
                    )}
                    ...
                    {!status.connected && (
                        <button onClick={handleConnect} disabled={processing || status.starting}
                            ...
                            {status.starting ? t('Menghubungkan...') : t('Hubungkan WhatsApp')}
                        </button>
                    )}
                    {status.connected && (
                ...
                <form onSubmit={handleSave} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-6 max-w-lg">
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('URL Service WhatsApp')}</label>
                        ...
                        <p className="text-xs text-slate-400 mt-1">{t('Alamat Node.js service whatsapp-web.js')}</p>
                    </div>
                    ...
                    <label className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                        ...
                        {t('Aktifkan WhatsApp Gateway')}
                    </label>

                    <div className="border-t border-slate-100 dark:border-slate-800 pt-4">
                        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">{t('Kirim Otomatis')}</h3>
                ...
                {status.connected && (
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 max-w-lg">
                        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">{t('Test Kirim Pesan')}</h3>
