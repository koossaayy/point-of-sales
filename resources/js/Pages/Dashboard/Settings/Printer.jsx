...
export default function Printer({ settings }) {
    ...
    const submit = (e) => {
        ...
        post(route("settings.printer.update"), {
            preserveScroll: true,
            onSuccess: () => toast.success(t('Pengaturan printer disimpan')),
    ...
    return (
        <>
            <Head title={t('Pengaturan Printer')} />
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <IconPrinter size={28} className="text-primary-500" />
                        {t('Pengaturan Printer')}
                    </h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t('Atur printer thermal untuk cetak struk otomatis')}</p>
                </div>
                ...
                <form onSubmit={submit} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-6 max-w-lg">
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('Ukuran Kertas')}</label>
                        <select value={data.printer_paper_size} onChange={(e) => setData("printer_paper_size", e.target.value)}
                            ...
                            <option value="80mm">{t('80 mm')}</option>
                            <option value="58mm">{t('58 mm')}</option>
                        </select>
                    </div>
                    ...
                    <label className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                        ...
                        {t('Cetak otomatis setelah transaksi')}
                    </label>

                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                        <p className="text-sm text-slate-500 mb-3">{t('Printer thermal terhubung via WebUSB (Chrome/Edge)')}</p>
