...
const STATUS_CONFIG = {
    submitted: { get label() { return i18next.t('Menunggu'); }, color: "bg-warning-100 text-warning-700 dark:bg-warning-900/50 dark:text-warning-400", icon: IconClock },
    accepted: { get label() { return i18next.t('Diterima'); }, color: "bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-400", icon: IconCheck },
    completed: { get label() { return i18next.t('Selesai'); }, color: "bg-success-100 text-success-700 dark:bg-success-900/50 dark:text-success-400", icon: IconCheck },
    rejected: { get label() { return i18next.t('Ditolak'); }, color: "bg-danger-100 text-danger-700 dark:bg-danger-900/50 dark:text-danger-400", icon: IconX },
    cancelled: { get label() { return i18next.t('Dibatalkan'); }, color: "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400", icon: IconX },
};

const PAY_CONFIG = {
    pay_at_counter: { get label() { return i18next.t('Bayar di Kasir'); }, color: "text-slate-600 dark:text-slate-400" },
    pay_online: { get label() { return i18next.t('Bayar Online'); }, color: "text-primary-600 dark:text-primary-400" },
};
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                                                ...
                                                <span>{t('Subtotal: Rp')} {Number(order.subtotal ?? 0).toLocaleString("id-ID")}</span>
                                ...
                                <div className="p-4">
                                        ...
                                        {(order.items ?? []).map((item) => (
                                            <div key={item.id} className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-slate-800 last:border-0">
                                                <div>
                                                    ...
                                                    {item.note && (
                                                        <p className="text-xs text-slate-400 mt-0.5">{t('Catatan:')} {item.note}</p>
                                                    )}
                                                </div>
                                    ...
                                    <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                        ...
                                        <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                                            {t('Total: Rp')} {Number(order.subtotal ?? 0).toLocaleString("id-ID")}
                                        </span>
                                    </div>
                                </div>
                            </div>
