                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            {t('Supplier: {{0}} • Dibuat oleh {{1}} • {{2}}', { 0: ret.supplier?.name || "-", 1: ret.creator?.name || "-", 2: formatDateTime(ret.created_at) })}
                        </p>
                        {ret.returned_at && (
                            <p className="text-sm text-slate-500">
                                {t('Diselesaikan: {{0}}', { 0: formatDateTime(ret.returned_at) })}
                            </p>
                        )}
                    </div>
                            ...
                            {ret.payable && (
                                <div className="flex justify-between">
                                    ...
                                    <Link
                                        ...
                                        className="font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400"
                                    >
                                        {t('{{0}} (Sisa: {{1}})', { 0: formatCurrency(ret.payable.total), 1: formatCurrency(ret.payable.total - ret.payable.paid) })}
                                    </Link>
                                </div>
                            )}
                            <div className="flex justify-between">
