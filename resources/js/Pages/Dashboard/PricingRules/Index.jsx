...
const targetLabel = (rule) => {
    if (rule.target_type === "product") return rule.product?.title || i18next.t('Produk');
    if (rule.target_type === "category") {
        return rule.category?.name || i18next.t('Kategori');
    }

    return i18next.t('Semua Produk');
};

const customerScopeLabel = (scope) => {
    if (scope === "walk_in") return i18next.t('Umum');
    if (scope === "registered") return i18next.t('Pelanggan');
    if (scope === "member") return i18next.t('Member');
    return i18next.t('Semua');
};

const kindLabel = (kind) => {
    if (kind === "qty_break") return i18next.t('Grosir');
    if (kind === "bundle_price") return i18next.t('Bundle');
    if (kind === "buy_x_get_y") return i18next.t('BXGY');
    return i18next.t('Standar');
};
