                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            {t('Supplier: {{0}} • Dibuat oleh {{1}} • {{2}}', { 0: order.supplier?.name || "-", 1: order.creator?.name || "-", 2: formatDateTime(order.created_at) })}
                        </p>
                        {order.ordered_at && (
                            <p className="text-sm text-slate-500">{t('Dipesan: {{0}}', { 0: formatDateTime(order.ordered_at) })}</p>
                        )}
                    </div>
