...
const coverageStatusConfig = {
    critical: {
        get label() { return i18n.t('Kritis'); },
        className:
            "bg-rose-100 text-rose-700 dark:bg-rose-950/50 dark:text-rose-300",
    },
    low: {
        get label() { return i18n.t('Rendah'); },
        className:
            "bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300",
    },
    healthy: {
        get label() { return i18n.t('Sehat'); },
        className:
            "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300",
    },
    no_movement: {
        get label() { return i18n.t('Tidak Bergerak'); },
        className:
...
const promoStatusConfig = {
    active: {
        get label() { return i18n.t('Aktif'); },
        className:
            "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300",
    },
    scheduled: {
        get label() { return i18n.t('Terjadwal'); },
        className:
            "bg-sky-100 text-sky-700 dark:bg-sky-950/50 dark:text-sky-300",
    },
    expired: {
        get label() { return i18n.t('Berakhir'); },
        className:
            "bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300",
    },
    inactive: {
        get label() { return i18n.t('Nonaktif'); },
        className:
            "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300",
    },
};
...
const promoKindLabel = {
    get standard_discount() { return i18n.t('Discount'); },
    get qty_break() { return i18n.t('Grosir'); },
    get bundle_price() { return i18n.t('Bundle'); },
    get buy_x_get_y() { return i18n.t('BXGY'); },
};

const crmCampaignTypeLabel = {
    get promo_broadcast() { return i18n.t('Promo Broadcast'); },
    get invoice_share() { return i18n.t('Invoice Share'); },
    get due_date_reminder() { return i18n.t('Due Reminder'); },
    get repeat_order_reminder() { return i18n.t('Repeat Order'); },
};
                ...
                <Table.Card title={t('CRM Operational Snapshot')}>
                    ...
                    <div className="mb-4 flex flex-wrap gap-2">
                        <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                            {t('Manual Segment: {{0}}', { 0: Number(
                                crmSummary.segments_manual ?? 0
                            ).toLocaleString("id-ID") })}
                        </span>
                        <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                            {t('Auto Segment: {{0}}', { 0: Number(
                                crmSummary.segments_auto ?? 0
                            ).toLocaleString("id-ID") })}
                        </span>
                        <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                            {t('Memberships: {{0}}', { 0: Number(
                                crmSummary.memberships_total ?? 0
                            ).toLocaleString("id-ID") })}
                        </span>
                        <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                            {t('Campaign Processed: {{0}}', { 0: Number(
