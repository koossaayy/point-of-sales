...
const statusBadge = (status) => {
    ...
    const labels = { paid: t('Lunas'), pending: t('Menunggu'), unpaid: t('Belum Lunas'), pending_approval: t('Menunggu Approval') };
...
export default function TransactionDetail({ transaction, token }) {
    ...
    return (
        <>
            <Head title={t('Invoice {{0}}', { 0: transaction.invoice })} />
            <div className="min-h-screen bg-slate-50 py-8 px-4">
                <div className="max-w-lg mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="bg-gradient-to-r from-primary-500 to-primary-700 px-6 py-5 text-white text-center">
                        <p className="text-sm opacity-80">{t('INVOICE')}</p>
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="border-t border-slate-100 pt-4">
                            <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-2">{t('Item')}</p>
                        ...
                        <div className="border-t border-slate-100 pt-4 space-y-1.5 text-sm">
                            <div className="flex justify-between text-slate-500"><span>{t('Subtotal')}</span><span>{formatPrice(transaction.grand_total + transaction.discount - (transaction.shipping_cost || 0) - (transaction.tax_total || 0))}</span></div>
                            {transaction.discount > 0 && <div className="flex justify-between text-slate-500"><span>{t('Diskon')}</span><span>-{formatPrice(transaction.discount)}</span></div>}
                            {transaction.tax_total > 0 && <div className="flex justify-between text-slate-500"><span>{t('PPN')}</span><span>{formatPrice(transaction.tax_total)}</span></div>}
                            {transaction.shipping_cost > 0 && <div className="flex justify-between text-slate-500"><span>{t('Ongkir')}</span><span>{formatPrice(transaction.shipping_cost)}</span></div>}
                        ...
                        {transaction.payment_method === "cash" && transaction.cash > 0 && (
                            <div className="border-t border-slate-100 pt-4 space-y-1.5 text-sm">
                                <div className="flex justify-between text-slate-500"><span>{t('Tunai')}</span><span>{formatPrice(transaction.cash)}</span></div>
                                {transaction.change > 0 && <div className="flex justify-between text-slate-500"><span>{t('Kembali')}</span><span>{formatPrice(transaction.change)}</span></div>}
                            </div>
                        )}

                        {transaction.receivable && transaction.receivable.status !== "paid" && (
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
                                <p className="text-sm font-semibold text-amber-800">{t('Sisa Pembayaran')}</p>
                                <div className="flex justify-between text-sm">
                                    <span className="text-amber-700">{t('Jatuh tempo: {{0}}', { 0: formatDate(transaction.receivable.due_date) })}</span>
